tchol_summary_stats <- function(df,finalValue){
	n_imp_below_thres <-  sum(df$TChol_Change == "Improved" & df$Final_result_below_threshold == "Yes")
	n_improved <- sum(df$TChol_Change == "Improved")
	n_no_change <- sum(df$TChol_Change == "No change")
	n_decreased <- sum(df$TChol_Change == "Not improved")
...
tchol_summary_bins <- function(df,initialValue){
	...
	for(val in seq(25,275,25)){
		...
		n_in_bin <- sum(val < df$TChol_Last_Result_Value & df$TChol_Last_Result_Value <= val+25)
